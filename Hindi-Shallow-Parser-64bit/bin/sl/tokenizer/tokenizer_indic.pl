...
GetOptions('help!'=>\$help, 'lang=s'=>\$lang, 'str_input=s'=>\$str_input, 'input=s'=>\$input, 'output:s'=>\$output, 'version'=>\$ver);
...
if($ver eq 1) {
        print "tokenizer-indic version 1.3 \n";
        exit(0);
}
...
sub tokenizer_indic {
	...
	if ($str_input ne "" and lc($lang) ne "") {
		my $tokenize_str = &sentence_mark($lang, $str_input, $acr_file);
		...
		#$line = join("",  <INFILE>);
		if ($line !~ m/^$/) {
			$tokenize_str = &sentence_mark($lang, $line, $acr_file);
