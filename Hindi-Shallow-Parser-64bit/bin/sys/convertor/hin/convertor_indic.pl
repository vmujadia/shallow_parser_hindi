...
GetOptions('help!'=>\$help, 'format=s'=>\$type, 'lang=s'=>\$lang, 'input=s'=>\$input,
	   'output'=>\$output, 'src_encoding=s'=>\$src, 'tgt_encoding=s'=>\$tgt, 'version'=>\$ver);
...
if($help eq 1)
{
	#print "convertor-indic version 1.2.0 \n";
...
if($ver eq 1)
{
        print "convertor-indic version 1.3.0 \n";
        exit(0);
}
		...
		while($line=<INFILE>)
        	{
                	...
                	($num,$lex,$pos,$fs) = split(/\t/,$line);
                        #$lexlang = &findlang($lex);
                        #print "$lexlang\n";
                        #if ($lexlang eq "eng")
                        #{
			    $lex_out = &wx2utf($lex, $lang);
                        #}
                        #else
                        #{
                        #    $lex_out = $lex;
